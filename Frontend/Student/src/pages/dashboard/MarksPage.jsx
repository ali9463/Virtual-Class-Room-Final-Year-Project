...
const generateGrades = (count) => {
  return Array.from({ length: count }, () => ({
    ...
    grade: faker.number.float({ min: 55, max: 100, fractionDigits: 1 }),
...
const MarksPage = () => {
  const [grades] = useState(generateGrades(15));
            ...
            <tbody>
              {grades.sort((a, b) => new Date(b.date) - new Date(a.date)).map((grade) => (
                <tr key={grade.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                  <td className="py-4 px-4 text-cyan-400 font-medium">{grade.subject}</td>
